...
public class CreateUser extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private void ValidatePhoneNumber(final String nametxt, final String empIdtxt, final String addresstxt, final String phoneNumbertxt, final String emailIdtxt, final String passwordtxt)
    {
        ...
        admin1=FirebaseDatabase.getInstance().getReference().child("Admin");
        admin1.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
                if(!(snapshot.child(phoneNumbertxt).exists()))
                {
                    HashMap<String,Object> userData=new HashMap<>();
                    userData.put("uid",uid);
                    admin1.child(phoneNumbertxt).updateChildren(userData)
        ...
        RootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            {
                if(!(dataSnapshot.child(uid).child(phoneNumbertxt).exists()))
                {
                    ...
                    RootRef.child(uid).child(phoneNumbertxt).updateChildren(userDataMap)
                            .addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
